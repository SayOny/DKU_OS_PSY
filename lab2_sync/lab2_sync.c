...
void enqueue_cg(queue_node *new_node) {
    ...
    pthread_mutex_unlock(&rear->lock);
}
...
void dequeue_cg(queue_node *del_node) {
    pthread_mutex_lock(&del_node->next->prev->lock);
    pthread_mutex_lock(&del_node->prev->next->lock);
    pthread_mutex_lock(&front->lock);
    pthread_mutex_lock(&rear->lock);
    ...
    pthread_mutex_unlock(&del_node->next->prev->lock);
    pthread_mutex_unlock(&del_node->prev->next->lock);
    pthread_mutex_unlock(&front->lock);
    pthread_mutex_unlock(&rear->lock);
}
...
void hash_queue_add_cg(hlist_node **hashtable, int val) {
    ...
    pthread_mutex_lock(&new_node->lock);
    pthread_mutex_lock(&new_hlist_node->q_loc->lock);
    pthread_mutex_lock(&new_hlist_node->next->lock);
    ...
    *hashtable = new_hlist_node;
    ...
    pthread_mutex_unlock(&new_node->lock);
    pthread_mutex_unlock(&new_hlist_node->q_loc->lock);
    pthread_mutex_unlock(&new_hlist_node->next->lock);
...
void hash_queue_delete_by_target_cg() {
    ...
    pthread_mutex_lock(&tmp->lock); // next를 하면 락이 안걸릴텐데...
    pthread_mutex_lock(&tmp->next->lock);
    pthread_mutex_lock(&tmp2->lock);
    pthread_mutex_lock(&hashlist[h]->lock);
    ...
    pthread_mutex_unlock(&tmp->next->lock);
    pthread_mutex_unlock(&tmp2->lock);
    pthread_mutex_unlock(&hashlist[h]->lock);
